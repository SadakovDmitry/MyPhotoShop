...
private:
    ...
    vec2i drag_offset;
public:
    ScrollBarSlider(vec2i pos_, vec2u size_, wid_t id_)
        : ABarButton(pos_, size_, id_),  is_dragging(false){
        //texture.create(static_cast<unsigned int>(size.x), static_cast<unsigned int>(size.y));
...
public:
    ScrollBarArrRight(vec2i pos_, vec2u size_, wid_t id_)
...
public:
    ScrollBarArrLeft(vec2i pos_, vec2u size_, wid_t id_)
        : ABarButton(pos_, size_, id_) {
